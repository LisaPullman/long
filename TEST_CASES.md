...
| 用例ID | 场景 | 操作 | 预期结果 |
|--------|------|------|----------|
| UI-I18N-001 | 首页一键切换 | 在首页点击右上角语言切换按钮 | 页面文案从中文切到英文（再次点击可切回） |
| UI-I18N-002 | 跨页面保持语言 | 切换到英文后依次进入订单列表/订单详情/订单创建/消息中心 | 所有页面文案保持英文，状态/筛选/按钮等均翻译 |
| UI-I18N-003 | 刷新后保持语言 | 切换到英文后刷新浏览器 | 刷新后仍保持英文（语言偏好持久化） |
| UI-I18N-004 | 时间显示随语言变化 | 英文模式下查看消息中心相对时间、订单创建/详情时间格式 | 相对时间文案为英文，日期格式使用英文 locale；中文同理 |
